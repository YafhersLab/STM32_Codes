...
#define IDR_PC13 *(volatile uint8_t *)(PERIPH_BB_BASE + BW_OFFSET(GPIOC_BASE + 0x10, 13))
#define IDR_PC3 *(volatile uint8_t *)(PERIPH_BB_BASE + BW_OFFSET(GPIOC_BASE + 0x10, 3))	//IDR -> 0X10, ODR -> 0X14
...
uint8_t button_read(uint8_t current){
	...
	static uint8_t lastState = 0;	//1 pull up, 0 pull down
	//filtrado
